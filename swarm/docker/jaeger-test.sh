...
case $1 in
	"start")
		...
		docker run -d --ip 10.1.2.11 --network swarminternal --rm -e PATH=/:/bin:/usr/bin:/usr/local/bin -e PASSWORD=tralala --name bzz1 -h bz1 $IMG --tracing --tracing.endpoint 10.1.2.2:6831 --bzzport 80 --httpaddr 0.0.0.0 --tracing.svc "bz1" 
		...
		;;
	"smoke")
		docker exec bzz1 /swarm-smoke --verbosity 5 --single --hosts 10.1.2.11,10.1.2.14 --filesize $FILESIZEK upload_and_sync 
		;;
	"status")
		for i in {1..4}; do
