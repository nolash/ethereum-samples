...
case $1 in
	"start_old")
		...
		for i in {1..4}; do
			docker run -d --ip 10.1.2.1$i --network swarminternal --rm -e PATH=/:/bin:/usr/bin:/usr/local/bin -e PASSWORD=tralala --name bzz$i -h bz$i swarm:trace --tracing --tracing.endpoint 10.1.2.2:6831 --sync-update-delay 1s --bzzport 80 --httpaddr 0.0.0.0 --tracing.svc "bz$i"
		done
		...
		echo "a$i: '$ENODENEW'"
		for i in {2..4}; do
			docker exec bzz$i /geth attach /root/.ethereum/bzzd.ipc --exec "admin.addPeer('$ENODENEW')"
			sleep 1
