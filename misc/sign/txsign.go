...
func init() {
	...
	flag.BoolVar(&g_tx, "x", false, "include tx hash in output")
	flag.BoolVar(&debug, "v", false, "Output debug information")
	flag.Parse()
...
func main() {
	...
	tx := types.NewTransaction(g_nonce, to, &wei, uint64(g_gasLimit), big.NewInt(int64(g_gasPrice)), []byte{})
