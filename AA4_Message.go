...
var (
	...
	quitOneC = make(chan struct{})
	quitTwoC = make(chan struct{})
)
...
var (
	proto = p2p.Protocol{
		...
		Run: func(p *p2p.Peer, rw p2p.MsgReadWriter) error {
			...
			var inmsgifc byte
			err = rlp.Decode(inmsg.Payload, &inmsgifc)
			if err != nil {
				return fmt.Errorf("Error in parsing incoming message payload: %v", err)
			}
			demo.Log.Debug("decoded", "payload", inmsgifc)
