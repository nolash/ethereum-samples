	...
	proto = p2p.Protocol{
		...
		Run: func(p *p2p.Peer, rw p2p.MsgReadWriter) error {
			...
			for !ponged {
				...
				if decodedmsg.Pong {
					log.Info("received pong", "peer", p)
					ponged = true
					pingW.Done()
				} else {
					log.Info("received ping", "peer", p)
					msg := FooPingMsg{
						Pong:    true,
						Created: time.Now(),
					}
...
func main() {
	...
	if err != nil {
		log.Crit("Generate private key #1 failed", "err", err)
	}
	privkey_two, err := crypto.GenerateKey()
	if err != nil {
		log.Crit("Generate private key #2 failed", "err", err)
	}
	...
	err = srv_one.Start()
	if err != nil {
		log.Crit("Start p2p.Server #1 failed", "err", err)
	}
		...
		for {
			...
			case peerevent := <-eventOneC:
				if peerevent.Type == "add" {
					log.Debug("Received peer add notification on node #1", "peer", peerevent.Peer)
				} else if peerevent.Type == "msgrecv" {
					log.Info("Received message nofification on node #1", "event", peerevent)
				}
			case <-sub_one.Err():
				return
			}
	...
	go func() {
		for {
			...
			case peerevent := <-eventTwoC:
				if peerevent.Type == "add" {
					log.Debug("Received peer add notification on node #2", "peer", peerevent.Peer)
				} else if peerevent.Type == "msgrecv" {
					log.Info("Received message nofification on node #2", "event", peerevent)
				}
			case <-sub_two.Err():
				return
			}
